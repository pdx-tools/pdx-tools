...
//     let rivers_bmp = std::fs::read(&rivers_bmp)?;
//     let rivers_bmp = Bmp::parse(&rivers_bmp)?;
...
//     let terrain_txt = std::fs::read(&terrain_txt)?;
...
//         .into_iter()
//         .map(|x| *definitions.get(&x)?)
//         .collect();

//     let trees_bmp = Path::new(map_dir).join("trees.bmp");
//     let trees_bmp = std::fs::read(&trees_bmp)?;
//     let trees_bmp = Bmp::parse(&trees_bmp)?;

//     let mut forest_colors = terrain.tree.get("forest").cloned()?.color;
//     forest_colors = forest_colors.to_vec();

//     let jungle_colors = terrain.tree.get("jungle").cloned()?.color;
...
//     let terrain_bmp = std::fs::read(&terrain_bmp)?;
//     let terrain_ind =
...
//             let predicted = province_terrain.get(&id).copied()?;
