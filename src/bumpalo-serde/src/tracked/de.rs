...
/// # Example
///
/// The `Deserializer` wraps any serde `Deserializer` to track the path to any
/// deserialization errors. Usage pattern:
///
/// ```ignore
/// use bumpalo_serde::tracked_deserialize;
...
/// match tracked_deserialize::<_, MyType>(deserializer, &mut path_buf) {
///     Ok(value) => { /* success */ },
///     Err(err) => {
    ...
    X: de::SeqAccess<'de>,
{
        ...
        T: DeserializeSeed<'de>,
    {
        ...
        self.index += 1;
        self.delegate
            .next_element_seed(TrackedSeed::new(seed, chain.clone(), track))
            .map_err(|err| track.trigger(&chain, err))
    }
