...
export async function loadSaveForRenderer(savePath: string, gamePath: string): Promise<string> {
...
export async function sendInteractionCursorMoved(x: number, y: number): Promise<void> {
  return await invoke<void>("interaction_cursor_moved", { x, y });
}

export async function sendInteractionMouseButton(button: number, pressed: boolean): Promise<void> {
...
export async function sendInteractionKey(code: string, pressed: boolean): Promise<void> {
