 ...
 * loading as the reader scrolls; without it, one page is shown.
 */
export function FeedList({ query, infinite = true }: { query: FeedQuery; infinite?: boolean }) {
  ...
  if (campaigns.length === 0) {
    return (
      <p className="rounded-lg border border-dashed border-gray-400/70 px-6 py-10 text-center text-gray-600 dark:text-gray-400">
        No {query.game ? `${GAME_LABEL[query.game]} ` : ""}saves shared yet.
      </p>
    );
  }
  ...
  return (
    <div className="flex flex-col gap-10">
      {campaigns.map((campaign) => (
        <FeedEntryCard key={`${campaign.game}:${campaign.key}`} campaign={campaign} />
      ))}
