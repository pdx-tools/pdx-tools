...
export type LedgerSelection = (worker: Eu4Worker, filter: CountryMatcher) => Promise<LedgerDatum[]>;

export function useLedgerData(fn: LedgerSelection) {
  const countryFilter = useTagFilter();
  return useAnalysisWorker(useCallback((worker) => fn(worker, countryFilter), [fn, countryFilter]));
}
