...
import { initSync, achievements, eu4_days_to_date, latest_eu4_minor_patch } from "../wasm/wasm_app";
...
export const loadAchievements = withWasm(() => achMemo ?? (achMemo = achievements()));
export const getAchievement = withWasm((id: number) => loadAchievements().find((x) => x.id == id));
