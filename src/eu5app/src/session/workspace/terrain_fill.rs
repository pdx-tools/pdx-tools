...
pub(crate) fn compute_surrounded_donors(
    ...
    let mut donors = LocationIndexedVec::filled(gpu_indices.len(), None);
    let mut visited = LocationIndexedVec::filled(gpu_indices.len(), false);
...
mod tests {
    ...
    fn build(
        ...
        let mut location_terrain = LocationIndexedVec::filled(n, Terrain::default());
        for (i, terrain) in terrains.iter().copied().enumerate() {
            let idx = LocationIdx::new(i as u32);
            gpu_indices[idx] = Some(GpuLocationIdx::new(i as u16));
            location_terrain[idx] = terrain;
        }
