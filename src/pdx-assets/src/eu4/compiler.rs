...
fn generate_provinces<P: FileProvider + ?Sized>(
    ...
    let impassable_provs: HashSet<_> =
    ...
    let calculated_terrains = map::calculate_province_terrains_with_precedence(
        &fs.read_file("map/terrain.txt")?,
        &fs.read_file("map/definition.csv")?,
        &fs.read_file("map/provinces.bmp")?,
        &fs.read_file("map/terrain.bmp")?,
        &fs.read_file("map/trees.bmp")?,
        override_precedence,
    )?;
    let total_provs = center_locations.len().saturating_sub(1);
